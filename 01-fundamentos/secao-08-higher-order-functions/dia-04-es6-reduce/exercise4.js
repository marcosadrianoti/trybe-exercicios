...
function studentAverage(grades, students) {
  let averageArray = grades.map(grade => grade.reduce((acc, curr) => acc += curr, 0) / grade.length);
  let resultArray = students.map((student, index) => ({ name: student, average: averageArray[index] }));
  return resultArray;
};
